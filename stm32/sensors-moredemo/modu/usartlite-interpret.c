...
void USART1_Lite_Init(u32 buad_rate)
{
	...
	// to set gpio partition
	GPIO_InitStrue.GPIO_Mode=GPIO_Mode_AF_PP; // OUTPUT
	GPIO_InitStrue.GPIO_Pin=GPIO_Pin_9;  // USART1_TX
	GPIO_InitStrue.GPIO_Speed=GPIO_Speed_10MHz;
    GPIO_Init(GPIOA,&GPIO_InitStrue);//②
	
	GPIO_InitStrue.GPIO_Mode=GPIO_Mode_IN_FLOATING;  // INPUT
	GPIO_InitStrue.GPIO_Pin=GPIO_Pin_10; // USART1_RX
	GPIO_InitStrue.GPIO_Speed=GPIO_Speed_10MHz;
    GPIO_Init(GPIOA,&GPIO_InitStrue);//②
	
	// to usart chip partition
	...
	// to set registers of usart chipset
